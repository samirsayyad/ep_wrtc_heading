...
exports.socketio = function (hookName, args, cb) {
	...
	io.of("/heading_chat_room").on("connect", function (socket) {
		...
		socket.on('getHeaderUserlist', (padId, headerId ,callback) => {
...
// TODO: look at the handleErrorStatMessage and find out when it's fire
 ...
 */
// TODO: security issue, we have to authorize the request befor handshaing
function handleRTCMessage (client, payload) {
	var padId = client.padId
	var msg = {
		data: {
			payload: {
				from: client.userId,
	...
	io.of("/heading_chat_room").to(padId).emit("RTC_MESSAGE", msg)
}
